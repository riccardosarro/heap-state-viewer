...
BREAK_FNS = ['malloc', 'calloc', 'realloc', 'free', 'valloc', 'pvalloc']
...
def compile_code():
  ...
  # TODO: Write the code to a file in a new Docker container
  tmp_dir = save_code_to_file(code)
  if tmp_dir is None:
    return jsonify({'error': 'Failed to save code to file'}), 500
  try:
    # TODO: Compile it with gcc
    os.system(f"gcc -o {tmp_dir}/{EXECUTABLE_NAME} -o {tmp_dir}/{FILENAME} -no-pie")
